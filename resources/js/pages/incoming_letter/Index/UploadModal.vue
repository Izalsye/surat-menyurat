...
const submit = () => {
    form.post(route('incoming-letter.store'), {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
...
const extract = () => {
    ...
    if (form.file !== null) {
        ...
        Promise.allSettled([
            axios.post(route('internal-api.incoming-letter.extract'), body),
            axios.post(route('internal-api.incoming-letter.category'), body)
        ]).then(responses => {
            ...
            if (extraction.status === 'fulfilled') {
                if (extraction.value.data) {
                    ...
                    if (extractedData) {
                        ...
                        // Parse tanggal dan atur jam ke tengah hari (jam 12)
                        const parsedDate = new Date(extractedData.tanggal_surat);
                        const parsedDateupload = new Date(Date.now());
                        parsedDate.setHours(12); // <-- Ini yang penting
                        form.letter_date = parsedDate;
                        form.sender = extractedData.pengirim;
                        form.institution = extractedData.instansi;
                        form.subject = extractedData.perihal;
                        form.body = extractedData.body_surat;
                        form.summary = extractedData.summary;
                        form.created_at = parsedDateupload;
                    }
                }
            }
