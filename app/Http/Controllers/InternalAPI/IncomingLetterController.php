...
class IncomingLetterController extends Controller
{
    public function extract(Request $request): JsonResponse
    {
        try {
            ...
            $prompt = <<<EOD
                Sebutkan informasi berupa nomor_surat, tanggal_surat, pengirim, instansi, perihal, body_surat, dan summary dari yang ada di file terlampir!
    ...
    public function disposition(Request $request): JsonResponse
    {
        try {
            if (!$request->hasFile('file'))
                throw new \Exception('File not found.');

            $urgency = implode(',', DispositionSeverity::values());
            $file = $request->file('file');
            $mime = $file->getMimeType();
            $base64 = base64_encode(file_get_contents($file->getRealPath()));
            $prompt = <<<EOD
                Aku ingin kamu menjawab dalam bentuk json dengan key description (string), urgency (string), dan reply_letter (boolean)
                Berdasarkan surat terlampir, aku ingin kamu membantuku untuk menentukan disposisi surat tersebut,
                description adalah isi dari disposisinya, urgency adalah salah satu di antara [$urgency], sedangkan reply_letter adalah apakah surat tersebut harus dibalas lagi,
                jangan beri code highlight! jangan diberi break point atau enter!
                dan jangan beri caption apa-apa!
            EOD;

            $response = GeminiHelper::generateTextFromTextAndImageInput($prompt, $mime, $base64);
            Log::debug(sprintf('GEMINI RESPONSE FROM %s@%s', self::class, 'disposition'), $response);
            $content = $response['candidates'][0]['content']['parts'][0]['text'];
            return response()->json([
                'error' => false,
                'message' => 'Success',
